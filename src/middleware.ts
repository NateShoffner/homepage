...
export async function middleware(req: NextRequest) {
  ...
  const loginUrl = `https://${process.env.CF_ACCESS_TEAM_DOMAIN}/cdn-cgi/access/login/${req.nextUrl.hostname}${req.nextUrl.pathname}?redirect_url=${encodeURIComponent(req.url)}`
